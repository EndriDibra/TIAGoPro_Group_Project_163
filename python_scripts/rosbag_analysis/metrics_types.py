...
class ScenarioMetrics:
    ...
    # Core metrics
    psc: float                # Personal Space Compliance (% timesteps >= 1.0m)
    min_ttc: float            # Minimum Time-to-Collision (seconds), inf if never approaching
    min_distance: float       # Minimum distance to human (meters)
    ...
    # Duration
    scenario_duration: float  # Total scenario time (seconds)
...
def write_metrics_csv(metrics: list[ScenarioMetrics], output_path: str):
    ...
    fieldnames = [
        ...
        'detection_latency', 'vlm_latency', 'scenario_duration'
    ]
    ...
    with open(output_path, 'w', newline='') as f:
        ...
        for m in metrics:
            writer.writerow({
                ...
                # Detection latency: -1.0 means no data, other values (including negative) are valid
